...
for epoch in range(1, max_epoch + 1):
    ...
    for i, batch in enumerate(trainloader):
        ...
        proto0, proto1 = model(trainx1, trainx2)
        center0 = proto0.mean(dim=0)
        center1 = proto1.mean(dim=0)
        
    
        label0 = batch[1].cuda()
        label1 = batch[3].cuda()
        
        #samples1 = model(trainx1, trainx2)
        ...
        dist0 = torch.stack((dist00,dist01), dim = 1).cuda()
        dist1 = torch.stack((dist11,dist10), dim = 1).cuda()
        
        dist = torch.cat((dist0, dist1), dim=0)
        label = torch.cat((label0, label1), dim=0)
        
        
        loss00 = criterion(center0, proto0, label0)
        loss01 = criterion(center0, proto1, label1)
        loss11 = criterion(center1, proto1, label1)
        loss10 = criterion(center1, proto0, label0)
        
        loss = loss00+loss01+loss11+loss10
