...
trainx1 = torch.from_numpy(np.array(X1)).float()
trainx1 = trainx1.permute(0,3,1,2).cuda()
y_train1 = torch.from_numpy(y1).float().cuda()

trainx2 = torch.from_numpy(np.array(X2)).float()
trainx2 = trainx2.permute(0,3,1,2).cuda()
y_train2 = torch.from_numpy(y2).float().cuda()
